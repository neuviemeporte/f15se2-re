...
void routine_96(int param_1, int param_2, int param_3) {
    ...
    curRecordIdx = 0;
    var_192 = drawFlightPath((int *)param_3, 0x270f);
...
void routine_27(void) {
    ...
    if (gameData[0x4e / 2] == 1) {
        ...
        var_84 = 0;
        for (p = 0; ; p++) {
            if (!(var_176[p] = ((char far *)gameData)[p + 2]))
                break;
        }
    ...
    if ((unsigned)gameData[0x20 / 2] >= 6)
        goto medals;
    if (*(long far *)&gameData[0x32 / 2] <= *(long *)&var_89[gameData[0x20 / 2] * 2])
        goto medals;
    gfx_jump_3d_null(6);
